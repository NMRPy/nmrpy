...
class Base:
    ...
    def _extract_procpar_varian(procpar):
        ...
        rt = at + d1
        nt = numpy.array([procpar["procpar"]["nt"]["values"]], dtype=int).flatten()
...
class Fid(Base):
    ...
    def _ppm(self):
        ...
        if self._params is not None and self.data is not None:
            return numpy.linspace(
                self._params["sw_left"] - self._params["sw"],
                self._params["sw_left"],
                len(self.data),
            )[::-1]
        else:
            return None
    ...
    def ranges(self, ranges):
        ...
        if ranges.shape[1] != 2:
            raise AttributeError(
                "ranges must be an iterable of 2-length iterables or an empty iterables e.g. [[]]"
            )
        for r in ranges:
    ...
    def _index_ranges(self):
        ...
        if self.ranges is not None:
            ...
            index_ranges = self._conv_to_index(
                self.data,
                self.ranges.flatten(),
                self._params["sw_left"],
                self._params["sw"],
            ...
            return numpy.array(
                [
                    [
                        ...
                        if peak > min(peak_range) and peak < max(peak_range)
                    ]
    ...
    def _get_plots(self):
        ...
        plots = [
            self.__dict__[id]
            for id in sorted(self.__dict__)
            if isinstance(self.__dict__[id], Plot)
        ]
        return plots
...
"""
        ...
        self._calibrate_widget = Calibrator(
            self,
            title=plot_title,
            label=plot_label,
        )
    ...
    def baseline_correct(self, deg=2):
        ...
        if self._bl_indices is None:
            raise AttributeError(
                "No points selected for baseline correction. Run fid.baseliner()"
            )
        if not len(self.data):
            raise AttributeError("data does not exist.")
    ...
    def peakpick(self, thresh=0.1):
        ...
        peaks_ppm = Fid._conv_to_ppm(
            self.data, peaks_ind, self._params["sw_left"], self._params["sw"]
        )
...
"""
        ...
        self._peakpicker_widget = DataPeakSelector(
            self,
            title=plot_title,
            label=plot_label,
        )
...
"""
        ...
        self._baseliner_widget = FidRangeSelector(
            self,
            title=plot_title,
            label=plot_label,
        )
    ...
    def _f_fitp(cls, data, peaks, frac_gauss=None, method="leastsq"):
        ...
        for parset in range(len(p)):
            ...
            for k, v in current_parset.items():
                ...
                params.add(name=par_name, value=v, vary=True, min=0.0)
...
class FidArray(Base):
    ...
    def get_fids(self):
        ...
        fids = [
            self.__dict__[id]
            for id in sorted(self.__dict__)
            if isinstance(self.__dict__[id], Fid)
        ]
        return fids
...
"""
        ...
        self._baseliner_widget = FidArrayRangeSelector(
            self, title=plot_title, label=plot_label, voff=0.01
        )
...
Left - select peak
"""
        self._calibrate_widget = RangeCalibrator(
            self,
...
Ctrl+Alt+Right - assign
"""
        self._peakpicker_widget = DataPeakRangeSelector(
            self,
            y_indices=fid_number,
            aoti=assign_only_to_index,
            voff=voff,
    ...
    def peakpicker_traces(self, voff=0.02, lw=1):
        """
...
Ctrl+Alt+Right - assign
"""
        self._select_trace_widget = DataTraceSelector(
            self,
    ...
    def get_integrals_from_traces(self):
        ...
        if not hasattr(self, "_integral_traces"):
            raise AttributeError(
                "No integral traces. First run select_integral_traces()."
            )
        integrals_set = {}
    ...
    def assign_integrals(self, integrals_set: list) -> dict:  # deprecated?
        ...
        return SelectMultiple(options=widget_list, description="Integrals:")
...
class BrukerImporter(Importer):
    ...
    def _get_time_delta(self):
        ...
        for i in range(self.incr):
            ...
            d1 = pp["D"][1]
            nt = pp["NS"]
            tot = (at + d1) * nt / 60.0  # convert to mins
            td += tot
            tcum.append(td)
            tsingle.append(tot)
